...
func genClientSkeleton(plugin *protogen.Plugin, f *protogen.File) error {
	...
	discoveryName := easymicroprotogen.MustGetProtocGenConfig().DiscoveryName
	schema := easymicroprotogen.MustGetProtocGenConfig().GRPCResolveSchema
	...
	if _, err = plugin.NewGeneratedFile(f.GeneratedFilenamePrefix+"_easymicro_grpc.pb.go", f.GoImportPath).Write(b.Bytes()); err != nil {
		log.Println(runtimeutil.NewStackErr(err))
		return err
	}
