...
这里用mgorm作为orm框架，需要下载https://github.com/995933447/mgorm/blob/main/proto/ext.proto 放在proto存放相对目录中。

如果需要自定义或者修改代码生成规则，则需要下载https://github.com/995933447/easymicro/blob/main/proto/ext.proto 文件，引入并自定义proto_gen_opts选项。
```go
syntax = "proto3";
...
protoc --go_out=. \
  --go-grpc_out=. \
  --mgorm_out=. \
  --easymicro-client_out=. \
  --easymicro-server_out=. \
  --proto_path=./proto \
  ./proto/user.proto
```
...
#### 全局文件需定义在执行protoc命令的工作目录的上一层目录名easymicro_loader中，如在/var/work中执行protoc，则全局文件请定义在/var/work/../easymicro_loader/即目录中即/var/easymicro_loader/。
配置文件可以是json、toml等格式，需要以protogen作为文件名，如/etc/easymicro/protogen.toml或者C:\easymicro\protogen.json等。
...
type ProtocGenConfig struct {
	...
	DiscoverPrefix              string `json:"discover_prefix" mapstructure:"discover_prefix"`                                 // 默认的服务注册前缀
