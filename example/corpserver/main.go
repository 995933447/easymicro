...
import (
	...
	"github.com/995933447/easymicro/example/corpserver/boot"
	"github.com/995933447/easymicro/example/corpserver/config"
	"github.com/995933447/easymicro/example/corpserver/event"
...
func main() {
	if err := boot.InitNode("corp"); err != nil {
	...
	if err := grpc.PrepareDiscoverGRPC(context.TODO(), corp.EasymicroGRPCSchema, corp.EasymicroDiscoveryName); err != nil {
		log.Fatal(runtimeutil.NewStackErr(err))
	}
	...
	err = grpc.ServeGRPC(context.TODO(), &grpc.ServeGRPCOptions{
		DiscoveryName:   corp.EasymicroDiscoveryName,
